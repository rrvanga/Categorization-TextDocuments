...
public class Main2 {
	...
	public static ArrayList<String> mFinalFrequentWords = new ArrayList<>();

	/*
	 * Application starts with main() here a window is created using Java API
	 */
	...
	/*
	 * All UI elements, variables are initialized, and actions for all buttons
	 * are defined.
	 */
	...
	private void initialize() {
		...
		frmTextMining.setTitle("Text Mining (using Xor)");
		...
		frmTextMining.getContentPane().add(lblProgress,
				"cell 1 3,alignx center,aligny center");
		final JProgressBar progressBar = new JProgressBar();
		progressBar.setStringPainted(true);
		progressBar.setMaximum(5);
		frmTextMining.getContentPane().add(progressBar,
				"cell 2 3,growx,aligny center");
		final JLabel lblNewLabel_1 = new JLabel("Select files and hit run");
		frmTextMining.getContentPane().add(lblNewLabel_1,
				"cell 2 4,alignx center,aligny bottom");
		final JSlider slider = new JSlider();
		slider.setMaximum(90);
		slider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				slider.setBorder(new TitledBorder(UIManager
						.getBorder("TitledBorder.border"), "0."
						+ slider.getValue(), TitledBorder.CENTER,
		...
		slider.setBorder(new TitledBorder(UIManager
				.getBorder("TitledBorder.border"), "0." + 50,
		...
		slider.setPaintLabels(true);
		frmTextMining.getContentPane().add(slider,
				"cell 2 1,growx,aligny center");
		...
		/*
		 * Action Listener for browse button. Here input files are taken and the
		 * paths are stored in an array.
		 */
		...
		btnRun.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				@SuppressWarnings("unchecked")
				ArrayList<String>[] inputForJaccard = new ArrayList[file.length] ;
				int nCounter = 0;
				...
				textArea_1.append("The frequent item set words are::: \n");
				nCounter = 0;
				for (ArrayList<String> arrayList : inputForJaccard) {
					System.out.print(nCounter + " ---->>  ");
					textArea_1.append(nCounter + " ---->>  ");
					for (String string : arrayList) {
...
//				starting XOR procedure
				
//				building frequent words for xor 
				...
				for (ArrayList<String> arrayList : docs) 
				{
					...
					for (String string : arrayList) 
					{
						...
						if (finalFrequentItems.contains(key))
						{
							if (!mFinalFrequentWords.contains(string)) {
								mFinalFrequentWords.add(string);
							}
						}
					}
				}				
				
				textArea_1.append("\n\nfinal frequent items (input for xor)"
						+ mFinalFrequentWords +"\n\n");
				
				
//				calling ModifiedXor class for building comparison table
				
				ModifiedXor mXor = new ModifiedXor();
				mXor.buildComparisonTable();
				
//				printing xor comparison table
				
				textArea_1.append("XOR comparision table \n\n\n");
				
				for( int i = 0; i < docs.length; i++)
				{
					textArea_1.append(nCounter + " ---->>  ");
					for(int j = 0 ; j < mFinalFrequentWords.size(); j++)
					{
						textArea_1.append(ModifiedXor.xorComparisonTable[i][j]+"\t");
					}
...
//				Calculating xor values for the documents andbuilding the second table
				
				mXor.buildXorValuesTable();
				
//				printing xorValuesTable
				...
				for( int i = 0; i < docs.length; i++)
				{
					...
					{
						textArea_1.append(ModifiedXor.xorValuesTable[i][j]+"\t");
					}
...
//				printing the final result : clusters
				...
				lblNewLabel_1.setText("Complete" + "Total execution time :"
	...
	public static void printFinalClusters(final JTextArea textArea_1) {		
		...
		int fCounter = 1;
		for( Multiset<Integer> temp : ModifiedXor.finalClusters){
